...
public class SendMessageService implements SendMessageUseCase {
    ...
    public SendMessageResponse sendMessage(SendMessageCommand command) throws UserNotFoundException, ChatRoomNotFoundException {
        ...
        List<UUID> userIds = optionalChatRoom.get().getMappings().stream().map(m -> m.getUser().getId()).toList();
        messageRepository.save(new Message(command.message(), optionalSender.get(), command.roomId()));
        for (UUID userId : userIds) {
            try {
                webSocketHandler.sendMessage(userId, command.message());
            } catch (IOException e) {
                continue;
            }
        }
